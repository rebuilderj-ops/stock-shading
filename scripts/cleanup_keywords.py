...
changed = 0
for d in data:
    if d.get('date') in ['2026-06-08', '2026-06-10', '2026-06-11'] and d.get('keywordName') not in existing_kws:
        old = d['keywordName']
        found = None
        for k_core, trigs in rules.items():
            if any(t.lower() in old.lower() or t.lower() in d.get('news_titles', '').lower() or t.lower() in d.get('reason', '').lower() for t in trigs):
                found = k_core
                break
        if not found:
            found = 'IT'
        ...
        print(f"Mapped: {old} -> {found} for {d['name']}")
        changed += 1

with open(f_path, 'w', encoding='utf-8') as f:
    json.dump(data, f, ensure_ascii=False, indent=4)

print(f"Successfully mapped {changed} keywords.")
